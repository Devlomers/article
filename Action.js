...
export const Actions = () => {
  ...
  const updateUser = (userData) => {
    fetch("http://localhost/php-react/update-user.php", {
      ...
      .then((data) => {
        if (data.success) {
          users = users.map((user) => {
            if (user.ar_no === userData.ar_no) {
              user.isEditing = false;
              user.ar_title = userData.ar_title;
              user.meta_desc = userData.meta_desc;
  ...
  const deleteUser = (theID) => {
    ...
    fetch("http://localhost/php-react/delete-user.php", {
      ...
      body: JSON.stringify({ ar_no: theID }),
    })
      .then((res) => {
        return res.json();
      })
